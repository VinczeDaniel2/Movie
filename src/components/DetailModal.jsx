...
export const DetailModal=({open,setOpen,id,media_type})=> {
    ...
    const {data:dataVideos,isLoading:isLoadingVideos,isError:isErrorVideos,error:errorVideos}=useQuery({queryKey:['details',urlVideos],queryFn:getData})
      ...
      >
        <Box sx={style}>
          ...
          {dataVideos?.results && dataVideos?.results.length>0 &&
            <div>
                <Button className='video' variant='contained' startIcon={<YouTubeIcon/>} target='blank' href={`https://www.youtube.com/watch?v=${dataVideos?.results[0].key}`}>
                Watch the trailer
